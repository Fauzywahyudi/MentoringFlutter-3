...
class EditBerita extends StatefulWidget {
  ...
  const EditBerita({@required this.model});
  @override
  _EditBeritaState createState() => _EditBeritaState();
}

class _EditBeritaState extends State<EditBerita> with BeritaValidation {
  ...
  @override
  void initState() {
    _tecJudul = TextEditingController(text: widget.model.judul);
    _tecIsi = TextEditingController(text: widget.model.isiBerita);
    _tecPenulis = TextEditingController(text: widget.model.penulis);
    _tecSumber = TextEditingController(text: widget.model.sumber);
    super.initState();
  }
        ...
        children: [
          Container(
            ...
            child: !_isImageChange
                ? _imagePicked(size, _isImageChange)
                : _isImageChange && _image == null
                    ? _noLogoPicked()
                    : _imagePicked(size, _isImageChange),
          ),
          Positioned(
  ...
  Widget _imagePicked(Size size, bool isAssetImage) => Container(
        width: size.width,
        child: ClipRRect(
          child: isAssetImage
              ? Image.file(
  ...
  void onSave() async {
    if (formKey.currentState.validate() &&
      ...
      if (_isImageChange) {
        ...
        if (isUploaded) {
          final model = Berita(
              idBerita: widget.model.idBerita,
              judul: _judul,
              isiBerita: _isi,
              penulis: _penulis,
              sumber: _sumber,
          ...
          final result = await provider.updateBerita(model);
          if (result) {
